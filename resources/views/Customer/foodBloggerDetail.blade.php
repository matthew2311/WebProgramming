        ...
        <div class="card mx-auto my-auto mt-5" style="width: 1000px; background-color: #EF8354">
            <div class="row no-gutters">
                ...
                @if ($foodBlogger->food_blogger_ig_link != null)
                    <a href="{{ $foodBlogger->food_blogger_ig_link }}" target="_blank" class="btn btn-light"><i
                            class="bi bi-instagram"></i> Instagram</a>
                @endif
                @if ($foodBlogger->food_blogger_tiktok_link != null)
                    <a href="{{ $foodBlogger->food_blogger_tiktok_link }}" target="_blank" class="btn btn-dark"><i
                            class="bi bi-tiktok"></i> TikTok</a>
                @endif
                @if ($foodBlogger->food_blogger_youtube_link != null)
                    <a href="{{ $foodBlogger->food_blogger_youtube_link }}" target="_blank" class="btn btn-danger"><i
                        ...
                        <div class="card mx-2 mt-2 mb-4 px-0" style="width: 15rem;">
                            ...
                            <div class="card-body">
                                <h5 class="card-title" style="color: #0D3B66">
                                    {{ Str::limit($fcr->restaurant->restaurant_name, 15) }}</h5>
                                <p class="card-text" style="color: #0D3B66">{{ $fcr->restaurant->category->category_name }}
                                </p>
                                ...
                                <p class="card-text" style="color: #0D3B66"><i class="bi bi-hand-thumbs-up-fill"></i>
                                    Direkomendasikan oleh {{ $fcr->foodBlogger->food_blogger_name }}</p>
